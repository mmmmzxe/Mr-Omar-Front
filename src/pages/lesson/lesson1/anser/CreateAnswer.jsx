...
const CreateAnswer = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // هنا سنستخدم XMLHttpRequest لضبط الـ progress
      const xhr = new XMLHttpRequest();
      xhr.open("POST", "https://omarroshdy.com/api/v1/ans");
      xhr.setRequestHeader("Authorization", `Bearer ${token}`);
      
      // حدث update للـ progress
      ...
      xhr.onload = () => {
        if (xhr.status === 201) {
          ...
          setAnswerText("");    // إعادة تعيين نص الإجابة
          setImageAsset(null);  // إعادة تعيين الصورة
          setIsCorrect("");     // إعادة تعيين اختيار الإجابة الصحيحة
          setProgress(0);       // إعادة تعيين النسبة
